document.addEventListener("DOMContentLoaded", function () {
  ...
  let currentQuiz = null;//za da razberem v koi quiz sme

  if (form) {
    form.addEventListener("submit", function (e) {  //za da ne se refreshva stranicata
      e.preventDefault();
      let errors = [];
  ...
  if (quizContainer && quizTitle) {
    ...
    if (selectedQuizApi) {
      ...
      if (!foundQuiz || !foundQuiz.questions || foundQuiz.questions.length === 0) {
        ...
        // Start quiz with local questions
        displayLocalQuestion(0, foundQuiz.questions, 0, []);
      }
    }
  }

  function displayQuestion(index, questions, score, answers) { //za api
  ...
  function showResults(score, totalQuestions, answers) {
    ...
    resultsHTML += "<h3>Detailed Results:</h3>";
    answers.forEach((ans, i) => {
      resultsHTML += `<div class="result-item">
          <p><strong>Question ${i + 1}:</strong> ${ans.question}</p>
          <p>Your answer: ${ans.selectedAnswer} ${ans.isCorrect ? "✔️" : "❌"}</p>`;
      if (!ans.isCorrect) {
        resultsHTML += `<p>Correct answer: ${ans.correctAnswer}</p>`;
      }
      resultsHTML += `</div><hr/>`;
    });
  ...
  if (createQuizButtonRedirection) { //redirectva ot avaliable kum create quiz
